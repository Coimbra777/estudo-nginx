...
Fazer o Nginx gerar e propagar um `X-Correlation-Id` por requisição para os três
destinos (`laravel`, `users_service`, `orders_service`), padronizar timeouts, e
devolver JSON em vez de HTML quando um upstream não responde. Esta feature é
pré-requisito de F03 (orders precisa repassar o correlation-id) e F04
(observabilidade precisa dele para correlacionar logs entre serviços).

## Escopo

- Usar `$request_id` (nativo do Nginx ≥ 1.11, presente em `nginx:alpine` atual) e
  propagar como `X-Correlation-Id` via `proxy_set_header`/`fastcgi_param` nos quatro
  blocos de `location` existentes (`/`, `/api/users/`, `/api/orders/`, `~ \.php$`).
...
- Rate limiting — não há indício de necessidade real neste ambiente de estudo;
  documentado como sugestão futura, não como feature aberta.
